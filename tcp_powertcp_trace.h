...
TRACE_EVENT(new_ack,
	TP_PROTO(u64 time, unsigned int hash, u32 ack_seq, u32 cwnd,
		unsigned long rate),
	TP_ARGS(time, hash, ack_seq, cwnd, rate),
	TP_STRUCT__entry(
		__field(u64, time)
...
TRACE_EVENT(norm_power,
	TP_PROTO(u64 time, unsigned int hash, long dt, long delta_t, long rtt_grad,
		long base_rtt, long power_scale, long p_norm, long p_smooth,
		long result),
	TP_ARGS(time, hash, dt, delta_t, rtt_grad, base_rtt, power_scale, p_norm,
		p_smooth, result),
	TP_STRUCT__entry(
		__field(u64, time)
		__field(unsigned int, hash)
	...
	TP_printk(
		"time=%llu us hash=%u: dt=%ld us delta_t=%ld us rtt_grad*%ld=%ld base_rtt=%ld us p_norm*%ld=%ld p_smooth*%ld=%ld => p_smooth*%ld=%ld",
...
TRACE_EVENT(update_window,
	TP_PROTO(u64 time, unsigned int hash, u32 cwnd_old, u32 cwnd,
		long power_scale, long p_norm, long beta, u32 result),
	TP_ARGS(time, hash, cwnd_old, cwnd, power_scale, p_norm, beta, result),
	TP_STRUCT__entry(
		__field(u64, time)
		__field(unsigned int, hash)
	...
	TP_printk(
		...
		__entry->time, __entry->hash, __entry->cwnd_old, __entry->cwnd,
